...
def create_dataframe(path_name:str)->pd.DataFrame:
    ...
    logging.info(f"\nData frame criado com {len(df)} linha(s)")
...
def normalize_datetime_columns(df: pd.DataFrame, colums_names:list[str]) -> pd.DataFrame:

    logging.info(f"\nConvertendo colunas para datetime:{colums_names}")
    for name in columns_names:
        df[name] = pd.to_datetime(df[name], unit='s', utc=True).dt.tz_convert('America/Brasilia')
